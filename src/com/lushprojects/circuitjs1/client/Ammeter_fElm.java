...
class AmmeterElm_ extends CircuitElm {
    ...
    public AmmeterElm_(int xx, int yy) { 
        super(xx, yy); 
        flags = FLAG_SHOWCURRENT|FLAG_CIRCLE;
        scale = SCALE_AUTO;
        // ---- NUEVO ----
        lockY = yy; // se bloquea en la posición inicial
        // ----------------
    }
    public AmmeterElm_(int xa, int ya, int xb, int yb, int f,
               StringTokenizer st) {
        super(xa, ya, xb, yb, f);
        scale = SCALE_AUTO;
